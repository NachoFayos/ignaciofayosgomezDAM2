window.onload = function(){
    ...
    let usuario = localStorage.getItem("bat2_usuario")
    if(usuario == undefined){
    	window.location = "../"
    }
    let token =  localStorage.getItem("bat2_token")
    if(token == undefined){
    	window.location = "../"
    }
        ...
        .then(data => {
            ...
            aplicaciones.forEach(function(aplicacion){                                      // Para cada una de las aplicaciones
                aplicacion.onclick = function(){                                            // Cuando haga click en esa aplicacion
                    localStorage.setItem('bat2_aplicacion', this.querySelector("p").textContent);
                    window.location = "../supercontrolador/"
                }
            })
        })
     document.querySelector("#cerrarsesion").onclick = function(){
     	localStorage.removeItem("bat2_usuario")
     		localStorage.removeItem("bat2_token")
     		window.location = "../"
     }
}