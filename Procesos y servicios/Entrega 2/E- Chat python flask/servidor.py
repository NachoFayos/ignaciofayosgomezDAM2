...
def toma():                                                 # Defino una funcion
    ...
    usuario = request.args.get('usuario')                   # atrapo el parametro de url llamado mensaje
    mensajes.append({'mensaje':mensaje,'usuario':usuario})  # Lo añado a la lista de mensajes
    return str({"mensaje":"ok"})                            # Devuelvo un json que dice ok

if __name__ == '__main__':                                  # Si esta es la clase principal
    app.run(debug=True, host='192.168.1.178')               # Ejecuta la aplicación y escucha en el puerto 5000, en la ip que indico
